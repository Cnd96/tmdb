...
angular.module('movieApp').controller('TvController',["$scope","$route","appDataService","$window" , function ($scope, $route,appDataService,$window) {
    ...
    function calculateInnerWidth(){
        if($window.innerWidth<767){
         angular.element('#tv-bg-poster-id').css('background-image', `linear-gradient(to right, rgba(var(--${$scope.linearColor}), 1), rgba(var(--${$scope.linearColor}), 0.17)),url('http://image.tmdb.org/t/p/w780${$scope.tvShowData.backdropPath}')`);
